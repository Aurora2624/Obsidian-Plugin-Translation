            ...
            o = [
              ...
              i(t.interaction.result, ["keyup", "input"], (t) => {
                (this.setColor(t.target.value, !0) &&
                  ...
                  (this._emit("更改", this._color, "input", this),
...
var Pickr = getDefaultExportFromCjs(pickr_min);
    ...
    plugins.forEach(function (plugin) {
      ...
      if (sortable[plugin.pluginName][eventNameGlobal]) {
        sortable[plugin.pluginName][eventNameGlobal](
          _objectSpread2({ sortable: sortable }, evt),
        );
      }
      if (
...
Sortable.prototype = {
  ...
  _prepareDragStart: function _prepareDragStart(evt, touch, target) {
    ...
    if (target && !dragEl && target.parentNode === el) {
      ...
      dragStartFn = function dragStartFn() {
        ...
        _dispatchEvent({ sortable: _this, name: "选择", originalEvent: evt });
  ...
  _dragStarted: function _dragStarted(fallback, evt) {
    ...
    if (rootEl && dragEl) {
      ...
      _dispatchEvent({ sortable: this, name: "开始", originalEvent: evt });
    } else {
      this._nulling();
    }
  },
  ...
  _onDragStart: function _onDragStart(evt, fallback) {
    ...
    _this.cloneId = _nextTick(function () {
      pluginEvent("克隆", _this);
      if (Sortable.eventCanceled) return;
      if (!_this.options.removeCloneOnHide) {
        rootEl.insertBefore(cloneEl, dragEl);
      }
      _this._hideClone();
      _dispatchEvent({ sortable: _this, name: "克隆" });
    });
...
class HighlightrSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl)
      .setName("选择高亮方法")
      .setDesc(
        `选择使用内联 CSS 或 CSS 类进行高亮。请注意，两种选择各有优缺点。内联 CSS 在导出笔记时能让您不依赖外部 CSS 文件。CSS 类则更灵活且易于自定义。`,
      )
    ...
    stylesSetting
      .setName("选择高亮样式")
      .setDesc(
        `根据您的设计审美，您可能希望自定义高亮样式。使用下拉菜单从多种高亮笔样式中选择。根据您的主题，此插件的 CSS 可能会被覆盖。`,
      )
      .addDropdown((dropdown) => {
        let styles = {};
    ...
    highlighterSetting
      ...
      .setDesc(
        `通过提供颜色名称并使用颜色选择器设置十六进制代码值来创建新的高亮颜色。退出颜色选择器前请勿忘记保存颜色。拖放高亮颜色可更改高亮器组件中的顺序。`,
      );
            ...
            __awaiter(this, void 0, void 0, function* () {
              ...
              if (color && value) {
                if (!this.plugin.settings.highlighterOrder.includes(color)) {
                  ...
                  new obsidian.Notice("该颜色已存在");
                }
              }
              color && !value
                ? new obsidian.Notice("高亮笔十六进制代码缺失")
                : !color && value
                  ? new obsidian.Notice("高亮笔名称缺失")
                  : new obsidian.Notice("高亮笔数值缺失");
            }),
          );
      });
...
const highlighterMenu = (app, settings, editor) => {
  ...
  if (editor && editor.hasFocus()) {
    ...
    if (editor.cursorCoords) {
      ...
      coords =
        (_c =
          (_b = (_a = editor.cm).coordsAtPos) === null || _b === void 0
            ? void 0
            : _b.call(_a, offset)) !== null && _c !== void 0
          ? _c
          : editor.coordsAtPos(offset);
    } else {
      return;
    }
    menu.showAtPosition({ x: coords.right + 25, y: coords.top + 20 });
  } else {
    new obsidian.Notice("焦点必须在编辑器中");
  }
};
...
function createStyles(settings) {
  ...
  setAttributes(styleSheet, { type: "text/css", id: "highlightr-styles" });
